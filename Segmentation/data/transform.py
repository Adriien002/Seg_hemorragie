...
def get_train_transforms(cfg):
   ...
   # 1. Chargement et formatage de base
        ...
        T.Spacingd(keys=["image", "seg"], pixdim=(1.0, 1.0, 1.0), mode=["trilinear", "nearest"]),
        ...
        T.SpatialPadd(keys=["image", "seg"], spatial_size=cfg["augmentation"]["spatial_size"], mode='constant', value=0.0),
        T.NormalizeIntensityd(keys=['image'], channel_wise=True, nonzero=cfg["augmentation"]["non_zero_norm"]),
        
    
        T.RandAffined(keys=['image', 'seg'],prob=0.5,rotate_range=math.radians(30),
                scale_range=0.1,
        ...
        T.RandCropByLabelClassesd(
                ...
                num_samples=cfg["augmentation"]["num_samples"],
                ratios =cfg["augmentation"]["class_ratios_per_dataset"],
                num_classes=len(cfg["augmentation"]["class_ratios_per_dataset"])
            )
            ])
